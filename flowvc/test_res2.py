"""Resemblyzer VC: quick train + test."""
...
class CV(nn.Module):
    def __init__(self):
        ...
        self.sp=nn.Sequential(nn.Linear(256,512),nn.GELU(),nn.Linear(512,512))
        self.ref=nn.Sequential(nn.Conv1d(512,512,3,padding=1),nn.GELU(),nn.Conv1d(512,512,3,padding=1))
    def forward(self,c,s):
        if c.shape[2]%2!=0: c=F.pad(c,(0,1))
        cz=self.down(c)
        sp=self.sp(s).unsqueeze(-1)  # (B,512,1)
        # Simple: add speaker channel to content
...
for s in spks:
    es=[]
    for u in utts[:3]:
        a=load_any(f'{base}/{s}/{s}_{u}_mic1.flac').numpy(); es.append(get_spk(a))
...
for s in spks:
    for u in utts:
        ...
        with torch.no_grad():
            z=mimi.encode_to_latent(a,quantize=False); c=mimi.quantizer.encode(z)
...
for s in spks:
    for t in spks:
        if s==t: continue
        for u in utts:
            if (s,u) in zq and (t,u) in zq: pairs.append((s,t,u))
random.shuffle(pairs)

opt=torch.optim.AdamW(list(ce.parameters())+list(cv.parameters()), lr=1e-3)
print(f'Training 20 steps ({len(pairs)} pairs)...')
t0=time.time()
for step in range(20):
    ...
    for s,t,u in pairs:
        ...
        zvc=cv(c_s,s_t); Tq=z_t.shape[2]
        zvc=zvc[:,:,:Tq]  # Trim to target z_q length
        T=min(zvc.shape[2],z_t.shape[2])
        loss=F.mse_loss(zvc[:,:,:T],z_t[:,:,:T])
        opt.zero_grad(); loss.backward()
        torch.nn.utils.clip_grad_norm_(list(ce.parameters())+list(cv.parameters()),1.0); opt.step()
        lt+=loss.item()
    print(f'  step {step:3d}: loss={lt/len(pairs):.4f}')

print(f'Done [{time.time()-t0:.1f}s]')

# Test
subprocess.run(['ffmpeg','-y','-i','/Users/asill/Downloads/origin.mp3','-ar','24000','-ac','1','-sample_fmt','s16','/tmp/tz3.wav'],capture_output=True)
src_a=load_any(f'{base}/p225/p225_001_mic1.flac').unsqueeze(0).unsqueeze(0)
tgt_p=load_any(f'{base}/p226/p226_001_mic1.flac')
tgt_c=load_any('/tmp/tz3.wav',dur=None)
...
for nm,ta in[('parallel',tgt_p),('cross',tgt_c)]:
    ...
    cs_cos=F.cosine_similarity(zv2[:,:,:T2].reshape(-1),zs[:,:,:T2].reshape(-1),dim=0)
    ct_cos=F.cosine_similarity(zv2[:,:,:T2].reshape(-1),zt2[:,:,:T2].reshape(-1),dim=0)
    print(f'{nm}: cos_src={cs_cos:.4f} cos_tgt={ct_cos:.4f} Δ={ct_cos-cs_cos:+.4f}')
