#!/usr/bin/env python3
"""MPS Mimi latency — patch nn.Conv1d for >65536 channel limit on MPS."""
import os
os.environ['PYTORCH_ENABLE_MPS_FALLBACK'] = '1'

# ── Patch nn.Conv1d BEFORE any model loading ──────────────────────────
import torch
import torch.nn as nn

_orig_conv1d_forward = nn.Conv1d._conv_forward

def _patched_conv_forward(self, input, weight, bias):
    if (input.device.type == 'mps' and 
        self.out_channels > 60000):
        input_cpu = input.to('cpu')
        weight_cpu = weight.to('cpu')
        bias_cpu = bias.to('cpu') if bias is not None else None
        result = _orig_conv1d_forward(self, input_cpu, weight_cpu, bias_cpu)
        return result.to('mps')
    return _orig_conv1d_forward(self, input, weight, bias)

nn.Conv1d._conv_forward = _patched_conv_forward
print("Patched nn.Conv1d for MPS >65536 channel limit")

# Also patch functional.conv1d as safety net
...
def _safe_conv1d(input, weight, bias=None, stride=1, padding=0, dilation=1, groups=1):
    if input.device.type == 'mps' and weight.shape[0] > 60000:
        return _orig_f_conv1d(input.to('cpu'), weight.to('cpu'),
...
lat=80+int(np.median(tt))
print("  Effective latency: ~{}ms (80ms frame + {:.0f}ms proc)".format(lat,np.median(tt)))

# Compare with CPU
print()
cpu_enc,cpu_dec,cpu_split,cpu_tot=18.7,9.7,0.4,28.6
mps_enc=np.median(te); mps_dec=np.median(td); mps_split=np.median(ts); mps_tot=np.median(tt)
print("  ─── CPU vs MPS (median) ───")
print("  Encode:  {:.1f}ms → {:.1f}ms ({:.1f}x)".format(cpu_enc,mps_enc,cpu_enc/mps_enc))
print("  Splitter:{:.1f}ms → {:.1f}ms ({:.1f}x)".format(cpu_split,mps_split,cpu_split/max(mps_split,0.01)))
print("  Decode:  {:.1f}ms → {:.1f}ms ({:.1f}x)".format(cpu_dec,mps_dec,cpu_dec/mps_dec))
print("  TOTAL:   {:.1f}ms → {:.1f}ms ({:.1f}x)".format(cpu_tot,mps_tot,cpu_tot/mps_tot))
